...
export class ExternalApiService {
  private readonly COUNTRIES_API = 'https://restcountries.com/v2/all?fields=name,capital,region,population,flag,currencies';
  ...
  async fetchCountries(): Promise<any[]> {
    try {
      const response = await firstValueFrom(
        this.httpService.get(this.COUNTRIES_API)
      );
